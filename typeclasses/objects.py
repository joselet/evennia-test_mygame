...
'''
modificaciones para hacer que los numeros y el plural hablen en español
'''
class Object(ObjectParent, DefaultObject):
    
    def get_numbered_name(self, count, looker, **kwargs):
        """
        Controla cómo se muestra el nombre cuando hay varios objetos iguales.
        """
        key = self.get_display_name(looker, **kwargs)
        
        if count <= 1:
            return key, key
        
        # Diccionario básico de números (puedes ampliarlo)
        numeros = {
            2: "dos", 3: "tres", 4: "cuatro", 5: "cinco", 
            10: "diez", 11: "once", 20: "veinte"
        }
        num_str = numeros.get(count, str(count))
        
        # Regla de plural básica en español:
        # Si termina en vocal, añade 's'. Si termina en consonante, añade 'es'.
        if key[-1] in "aeiouáéíóú":
            plural = key + "s"
        else:
            plural = key + "es"
            
        # Retornamos (singular, plural_con_numero)
        return key, f"{num_str} {plural}"
